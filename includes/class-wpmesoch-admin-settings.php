...
class Wpmesoch_Admin_Settings{
    public function __construct()
    {
        add_action('admin_menu', [$this, 'wpmesoch_add_menu_page']);
        add_action('admin_init', [$this, 'wpmesoch_register_settings']);
        add_action('admin_enqueue_scripts', [$this, 'wpmesoch_enqueue_admin_scripts']);
    }

    public function wpmesoch_add_menu_page()
    {
        add_menu_page(
            ...
            [$this, 'wpmesoch_settings_page'],
            'dashicons-format-chat'
        );
    }

    public function wpmesoch_enqueue_admin_scripts($hook) {
        ...
        wp_enqueue_style('wpmethods-social-chat-floating-icons-admin', WPMESOCH_WPMETHODS_URL . 'assets/css/admin' . $min_suffix . '.css', [], WPMESOCH_PLUGIN_VERSION);
    
        // Enqueue Admin JS 
        wp_enqueue_script('wpmethods-social-chat-floating-icons-admin', WPMESOCH_WPMETHODS_URL . 'assets/js/admin' . $min_suffix . '.js', ['jquery', 'wp-color-picker'], WPMESOCH_PLUGIN_VERSION, true);
    
        // Now localize the script (AFTER enqueue)
        $links = get_option('wpmesoch_settings');
        $existing_links = !empty($links['wpmesoch_links']) ? count($links['wpmesoch_links']) : 0;
    
        wp_localize_script('wpmethods-social-chat-floating-icons-admin', 'wpmesoch_data', [
        ...
        wp_enqueue_script('wpmethods-social-chat-floating-icons-sortable', WPMESOCH_WPMETHODS_URL . 'assets/js/Sortable.min.js', ['jquery'], '1.15.6', true);
    }
    


    public function wpmesoch_register_settings()
    {
        register_setting('wpmesoch_settings_group', 'wpmesoch_settings', [
            'sanitize_callback' => [$this, 'wpmesoch_sanitize_settings']
        ]);
    }

    private function wpmesoch_sanitize_select_position($input) {
        // Sanitize the input
    ...
    public function wpmesoch_sanitize_settings($settings){
        $limit = apply_filters('wpmethods_social_chat_link_limit', 2);
        if (isset($settings['wpmesoch_links']) && is_array($settings['wpmesoch_links'])) {
            //If limit is active (PRO inactive), cut extra items
            if (count($settings['wpmesoch_links']) > $limit) {
                $settings['wpmesoch_links'] = array_slice($settings['wpmesoch_links'], 0, $limit);
            }
            foreach ($settings['wpmesoch_links'] as &$link) {
        ...
        $settings['position'] = $this->wpmesoch_sanitize_select_position($settings['position'] ?? '');
    ...
    public function wpmesoch_settings_page()
    {
        ...
        <div class="wrap">
            ...
            <div class="wpmesoch-admin-tabs">
                <button class="wpmesoch-admin-tab-button active" data-tab="general"><?php esc_html_e('Icons & Links', 'wpmethods-social-chat-floating-icons'); ?></button>
                <button class="wpmesoch-admin-tab-button" data-tab="style"><?php esc_html_e('Widget Styles', 'wpmethods-social-chat-floating-icons'); ?></button>
                <?php if(function_exists('is_woocommerce')) { ?>
                <button class="wpmesoch-admin-tab-button" data-tab="woocommerce"><?php esc_html_e('WooCommerce', 'wpmethods-social-chat-floating-icons'); ?></button>
                <?php } ?>
                <button class="wpmesoch-admin-tab-button" data-tab="help"><?php esc_html_e('Help', 'wpmethods-social-chat-floating-icons'); ?></button>
            </div>

            <form method="post" action="options.php">
                <?php settings_fields('wpmesoch_settings_group'); ?>
                <?php do_settings_sections('wpmesoch_settings_group'); ?>

                <div class="wpmesoch-admin-tab-content active" id="tab-general">
                    <div id="wpmesoch-admin-repeater-fields">
                        <?php 
                        $limit = apply_filters('wpmethods_social_chat_link_limit', 2);
                        if (!empty($wpmesoch_links)) : ?>
                            <?php 
                            $displayed_count = 0;
                            foreach ($wpmesoch_links as $index => $link) : 
                                ...
                                ?>
                                <div class="wpmesoch-admin-field-group">
                                    <div class="wpmesoch-admin-preview-icon">
                                    ...
                                    <div class="flex-wpmesoch-admin">
                                        ...
                                        <input type="text" id="cl-color-<?php echo esc_attr($index); ?>" name="wpmesoch_settings[wpmesoch_links][<?php echo esc_attr($index); ?>][color]" value="<?php echo esc_attr($link['color'] ?? ''); ?>" class="color-picker" placeholder="Pick Icon Color" />
                                    </div>

                                    <div class="flex-wpmesoch-admin">
                                        <label for="cl-url-<?php echo esc_attr($index); ?>" class="cl-label-wpm">Enter URL</label>
                                        <input type="text" id="cl-url-<?php echo esc_attr($index); ?>" name="wpmesoch_settings[wpmesoch_links][<?php echo esc_attr($index); ?>][url]" value="<?php echo esc_url($link['url'] ?? ''); ?>" placeholder="URL (Ex: https://wa.me/88017900000)" />
                                    </div>

                                    <div class="flex-wpmesoch-admin">
                                        <label for="cl-class-<?php echo esc_attr($index); ?>" class="cl-label-wpm">Enter Icon Class</label>
                                        <input type="text" id="cl-class-<?php echo esc_attr($index); ?>" name="wpmesoch_settings[wpmesoch_links][<?php echo esc_attr($index); ?>][icon]" value="<?php echo esc_attr($link['icon'] ?? ''); ?>" class="icon-input" placeholder="Icon class (Ex: fas fa-home)" />
                                    </div>

                                    <div class="flex-wpmesoch-admin">
                                        <label for="cl-label-<?php echo esc_attr($index); ?>" class="cl-label-wpm">Enter Label</label>
                                        <input type="text" id="cl-label-<?php echo esc_attr($index); ?>" name="wpmesoch_settings[wpmesoch_links][<?php echo esc_attr($index); ?>][label]" value="<?php echo esc_attr($link['label'] ?? ''); ?>" placeholder="Enter Label (Ex: Whatsapp)" />
                                    </div>

                                    <div class="flex-wpmesoch-admin">
                                        <label for="cl-bgcolor-<?php echo esc_attr($index); ?>" class="cl-label-wpm">Background Color</label>
                                        <input type="text" id="cl-bgcolor-<?php echo esc_attr($index); ?>" name="wpmesoch_settings[wpmesoch_links][<?php echo esc_attr($index); ?>][bg_color]" value="<?php echo esc_attr($link['bg_color'] ?? ''); ?>" class="color-picker" placeholder="Pick Background Color" />
                                    </div>

                                    <div class="flex-wpmesoch-admin">
                                        <label for="cl-sbgcolor-<?php echo esc_attr($index); ?>" class="cl-label-wpm">Gradient Color</label>
                                        <input type="text" id="cl-sbgcolor-<?php echo esc_attr($index); ?>" name="wpmesoch_settings[wpmesoch_links][<?php echo esc_attr($index); ?>][s_bg_color]" value="<?php echo esc_attr($link['s_bg_color'] ?? ''); ?>" class="color-picker" placeholder="Pick Background Color" />
                                    </div>

                                    <button type="button" class="wpmesoch-admin-remove-field">Remove</button>
                                </div>
                        ...
                        <?php else : ?>
                            <div class="wpmesoch-admin-field-group">
                                ...
                                <div class="flex-wpmesoch-admin">
                                    ...
                                    <input type="text" id="cl-color-0" name="wpmesoch_settings[wpmesoch_links][0][color]" class="color-picker" placeholder="Pick Icon Color" />
                                </div>

                                <div class="flex-wpmesoch-admin">
                                    <label for="cl-url-0" class="cl-label-wpm">Enter URL</label>
                                    <input type="text" id="cl-url-0" name="wpmesoch_settings[wpmesoch_links][0][url]" placeholder="URL (Ex: https://wa.me/88017900000)" />
                                </div>

                                <div class="flex-wpmesoch-admin">
                                    <label for="cl-class-0" class="cl-label-wpm">Enter Icon Class</label>
                                    <input type="text" id="cl-class-0" name="wpmesoch_settings[wpmesoch_links][0][icon]" class="icon-input" placeholder="Icon class (Ex: fas fa-home)" />
                                </div>

                                <div class="flex-wpmesoch-admin">
                                    <label for="cl-label-0" class="cl-label-wpm">Enter Label</label>
                                    <input type="text" id="cl-label-0" name="wpmesoch_settings[wpmesoch_links][0][label]" placeholder="Enter Label (Ex: Whatsapp)" />
                                </div>

                               
                                <div class="flex-wpmesoch-admin">
                                    <label for="cl-bgcolor-0" class="cl-label-wpm">Background Color</label>
                                    <input type="text" id="cl-bgcolor-0" name="wpmesoch_settings[wpmesoch_links][0][bg_color]" class="color-picker" placeholder="Pick Background Color" />
                                </div>

                                <div class="flex-wpmesoch-admin">
                                    <label for="cl-sbgcolor-0" class="cl-label-wpm">Gradient Color</label>
                                    <input type="text" id="cl-sbgcolor-0" name="wpmesoch_settings[wpmesoch_links][0][s_bg_color]" class="color-picker" placeholder="Pick Gradient Color" />
                                </div>
                    ...
                    <button type="button" id="wpmesoch-admin-add-field" style="display: none">Add Link</button>
                    <?php if ( ! defined( 'WPMESOCH_PRO_VERSION' ) ) : ?>
                ...
                <div class="wpmesoch-admin-tab-content" id="tab-style">
                    <table class="form-table">
                        <tr valign="top">
                                <th scope="row"><?php esc_html_e('Widget Icon', 'wpmethods-social-chat-floating-icons'); ?></th>
                                <td><input type="text" name="wpmesoch_settings[toggle_icon_class]" value="<?php echo esc_attr($options['toggle_icon_class'] ?? 'fas fa-comment-dots'); ?>" placeholder="Ex: fas fa-comment-dots" /></td>
                        </tr>
                        <tr valign="top">
                            <th scope="row"><?php esc_html_e('Button Background', 'wpmethods-social-chat-floating-icons'); ?></th>
                            <td><input type="text" name="wpmesoch_settings[toggle_bg_color]" value="<?php echo esc_attr($options['toggle_bg_color'] ?? '#00DA62'); ?>" class="color-field" /></td>
                        </tr>
                        <tr valign="top">
                            <th scope="row"><?php esc_html_e('Gradient Background', 'wpmethods-social-chat-floating-icons'); ?></th>
                            <td><input type="text" name="wpmesoch_settings[toggle_gbg_color]" value="<?php echo esc_attr($options['toggle_gbg_color'] ?? ''); ?>" class="color-field" /></td>
                        </tr>
                        <tr valign="top">
                            <th scope="row"><?php esc_html_e('Icon Color', 'wpmethods-social-chat-floating-icons'); ?></th>
                            <td><input type="text" name="wpmesoch_settings[icon_color]" value="<?php echo esc_attr($options['icon_color'] ?? '#FFFFFF'); ?>" class="color-field" /></td>
                        </tr>
                        <tr valign="top">
                            <th scope="row"><?php esc_html_e('Icon Size (px)', 'wpmethods-social-chat-floating-icons'); ?></th>
                            <td><input type="number" name="wpmesoch_settings[icon_size]" value="<?php echo esc_attr($options['icon_size'] ?? '30'); ?>" /></td>
                        </tr>
                        <tr valign="top">
                            <th scope="row"><?php esc_html_e('Button Size (px)', 'wpmethods-social-chat-floating-icons'); ?></th>
                            <td><input type="number" name="wpmesoch_settings[height_width]" value="<?php echo esc_attr($options['height_width'] ?? '50'); ?>" /></td>
                        </tr>
                        <tr valign="top">
                            <th scope="row"><?php esc_html_e('Hover Color', 'wpmethods-social-chat-floating-icons'); ?></th>
                            <td><input type="text" name="wpmesoch_settings[hover_color]" value="<?php echo esc_attr($options['hover_color'] ?? '#128C7E'); ?>" class="color-field" /></td>
                        </tr>

                        <tr valign="top">
                            <th scope="row"><?php esc_html_e('Pulse Animation Circle', 'wpmethods-social-chat-floating-icons'); ?></th>
                            <td><input type="text" name="wpmesoch_settings[pulse_animation_border_color]" value="<?php echo esc_attr($options['pulse_animation_border_color'] ?? '#00DA62'); ?>" class="color-field" /></td>
                        </tr>
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                <textarea name="wpmesoch_settings[custom_text]" rows="4" cols="50" placeholder="Ex: Hi, how are you?" ><?php echo esc_attr($options['custom_text'] ?? ''); ?></textarea>
                            </td>
                        </tr>
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                <select name="wpmesoch_settings[position]">
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                <input type="text" name="wpmesoch_settings[bottom_offset]" value="<?php echo esc_attr($options['bottom_offset'] ?? '20px'); ?>" placeholder="Ex: 20px or 5%" />
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                <input type="text" name="wpmesoch_settings[left_offset]" value="<?php echo esc_attr($options['left_offset'] ?? '20px'); ?>" placeholder="Ex: 20px" />
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                <input type="text" name="wpmesoch_settings[right_offset]" value="<?php echo esc_attr($options['right_offset'] ?? '20px'); ?>" placeholder="Ex: 20px" />
                ...
                <?php if(function_exists('is_woocommerce')) { ?>
                <div class="wpmesoch-admin-tab-content" id="tab-woocommerce">
                <table class="form-table">
                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Track WooCommerce Product', 'wpmethods-social-chat-floating-icons'); ?></th>
                        <td>
                            <input type="checkbox"
                                name="wpmesoch_settings[track_woo_product]"
                                value="1"
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <textarea name="wpmesoch_settings[woo_product_msg]" rows="4" cols="50" placeholder="I want to buy this product"><?php echo esc_textarea($options['woo_product_msg'] ?? 'I want to buy this product and product details below'); ?></textarea>
                    ...
                    <tr valign="top">
                        ...
                        <td><input type="text" name="wpmesoch_settings[woo_product_name]" value="<?php echo esc_attr($options['woo_product_name'] ?? 'Product Name'); ?>" placeholder="Product Name" /></td>
                    </tr>


                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Product price label', 'wpmethods-social-chat-floating-icons'); ?></th>
                        <td><input type="text" name="wpmesoch_settings[woo_product_price]" value="<?php echo esc_attr($options['woo_product_price'] ?? 'Price'); ?>" placeholder="Price" /></td>
                    </tr>
                ...
                <div class="wpmesoch-admin-tab-content" id="tab-help">
